...
export class MockGoalRepository implements IGoalReporistory {
  ...
  async searchGoalsByNames(goalNames: string[]): Promise<Goal[]> {
    ...
    return legacyGoals.map((lg) => _legacy_goal_to_goal(lg, allTricks));
  }
  ...
  async getAllGoals(): Promise<Goal[]> {
    ...
    for (const legacyGoal of LEGACY_SOURCE_GOALS) {
      const goal = await _legacy_goal_to_goal(legacyGoal, allTricks);
      goals.push(goal);
    }
 ...
 */
const _legacy_goal_to_goal = (legacyGoal: LegacyGoal, allTricks: Trick[]) => {
 ...
 */
const LEGACY_SOURCE_GOALS: LegacyGoal[] = [
  {
